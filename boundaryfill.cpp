...
void bound_it(int x, int y, float* fillColor, float* bc) {
    ...
    // If current pixel is neither border color nor fill color
...
void mouse(int btn, int state, int x, int y) {
    if (btn == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        float bCol[] = { 0, 0, 1 }; // Border color
        float color[] = { 1, 0, 0 }; // Fill color (red)
        bound_it(x, 480 - y, color, bCol); // Flip y-coordinate
    }
}
